defmodule SeatMachine do
  ...
  def handle_event(:cast, {:tick}, {:full, _, {n, 0}} = state, data) when n >= 5 do
...
defmodule SeatLayout do
  ...
  def indexes_in_line({row, col}, {r_off, c_off}) do
    Stream.iterate({row + r_off, col + c_off}, fn {r, c} -> {r + r_off, c + c_off} end)
  end

  def index_in_bounds?({row, col}, {r_max, c_max}) do
    (row in 0..r_max) and (col in 0..c_max)
  end

  def first_index_in_line(seats, dims, pos, offset) do
    indexes_in_line(pos, offset)
    |> Stream.take_while(&index_in_bounds?(&1, dims))
    |> Enum.find(&MapSet.member?(seats, &1))
  end

  def at_neighbors(seats, dims, map, pos) do
    neighbor_indexes()
    |> Enum.flat_map(fn offset ->
      case first_index_in_line(seats, dims, pos, offset) do
        nil -> []
...
Enum.each(seat_pids, fn {pos, pid} -> SeatMachine.neighbors(pid, SeatLayout.at_neighbors(seats, dims, seat_pids, pos)) end)
